    ...
    (answer: any) => {
      ...
      switch (currentQuestion.type) {
        case "multiple-choice":
          ...
          if (correctOptions && correctOptions.length > 0) {
            // Check if lengths match and every item matches (Set equality)
            isCorrect =
              correctOptions.length === userSelection.length &&
              correctOptions.every((id) => userSelection.includes(id));
          } else {
            // Fallback for simple string match if options aren't tagged
            isCorrect = answer === currentQuestion.correctAnswer;
          }
          break;

        case "sorting":
          // Compare two arrays of strings (order matters)
          if (
        ...
        case "graphing":
          ...
          // We check if every correct point exists in the user's answer
        ...
        case "number-line":
          // Compare numbers with exact match
          const targetVal = currentQuestion.numberLineConfig?.correctValue;
          if (typeof targetVal === "number") {
            isCorrect = Number(answer) === targetVal;
          }
          break;
        case "vertical-multiplication": // <--- Add this case here
        case "fill_blank":
          // Validate object against expected answers
          // answer is { blank1: "8", blank2: "80" }
          const expected = currentQuestion.answerConfig?.expectedAnswers;
          console.log("expected", expected);
          if (expected && typeof answer === "object") {
            // Check if EVERY expected answer matches the user's input
            const allMatches = expected.every((exp) => {
              const userVal = answer[exp.id];
              if (!userVal) return false;
              // Normalize and compare
              return Number(userVal) === Number(exp.value);
            });
            // Also ensure user didn't miss any blanks
            const filledCount = Object.keys(answer).length;
            isCorrect = allMatches && filledCount >= expected.length;
          }
          break;
        ...
        case "equation":
        default:
          // Standard String comparison with normalization
          const normalizedInput = String(answer).trim().toLowerCase();

          if (currentQuestion.acceptableAnswers) {
            // Check against list of valid variations (e.g. "0.5", ".5", "1/2")
            ...
            isCorrect = normalizedInput === correctNormalized;
          }
          break;
      }
      ...
      if (isCorrect) {
        ...
        setTimeout(() => {
          ...
          setLastUserAnswer(null); // Reset to null for complex types
          loadNextQuestion();
        }, 1500);
      } else {
        ...
        // Scroll to solution explanation
        setTimeout(() => {
